...
%(squishrunner)s --testsuite "%(testsuite)s" %(testcase)s --reportgen xml2.1,"%(reportgen)s" --aut %(name_of_aut)s
%(squishserver)s --stop
"""
...
class RunSquish:
    ...
    def behavior_check(self):
        ...
        if suite_name == "suite_BE_01_SSO":
            ...
            cmd_lines_b = [r'svn export http://d25315/auto/bqs_scripts/trunk  --username=public --password=lattice --force',
                           r'call scan_result.bat']
        else:
            cmd_lines_a = [r'cp ./case/*.ini ./log/',
                           r'cp ./case/run.bat ./log/',]
            cmd_lines_b = [r'svn export http://d25315/auto/trunk   --username=public --password=lattice --force',
                           r'call run.bat']
        sts = 0
