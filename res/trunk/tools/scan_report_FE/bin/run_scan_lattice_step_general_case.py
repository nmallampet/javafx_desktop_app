...
def get_default_implementation(xml_file):
    ...
    for id1,node in enumerate(lst_node):
        if node.tag.lower() == 'baliproject':
            ...
            pass
        if node.attrib.has_key('default_implementation')>0:
...
class scan_general_case:
    ...
    def scan_set(self):
        ...
        self.scan_mrp = tool_scan_lattice.ScanLatticeMrp()
        self.scan_twr = tool_scan_lattice.ScanLatticeTwr(self.pap)
        self.scan_time = tool_scan_lattice.ScanLatticeTime()
    ...
    def write_report(self):
        self.for_bqs_data = '<scan_case>\n'
        all_time_data = dict(self.srr_data.items()+self.mrp_data.items()+self.par_data.items() + self.real_cpu_total.items() )
        data_list = []
    ...
    def scan_report(self):
        if self.flow == 'general':
            ...
            pass
        if self.conf_file:
            self.process_report(self.conf_file,self.report_file)
        ...
        for design in self.all_designs:
            ...
            for dir in os.listdir(design_path): # dir:Target_Fmax_is_060MHz   
                ...
                if 1:
                     ...
                     par_file = get_unique_file([useful_dir, ".par"])
                     clk_name = ''
                     design_clk_keys = design_clk.keys()
    ...
    def process_report(self,conf_file,report_file):
