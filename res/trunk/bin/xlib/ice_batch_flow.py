...
class Run_iCEcube:
    ...
    def get_lse_source_files(self, src_files):
        ...
        for item in src_files:
            ...
            if pre_str == "-constraint":
                temp_line = '-sdc "%s"'%src_file
                lse_source_files.append(temp_line)
                continue
            if len(item) > 1:
    ...
    def prepare_run_flow(self):
        ...
        if use_ice_prj:
            ...
            if  _devkit and _devkit_temp: #----------------------this may be a bug-------------
                ...
                # at here you need to use absolute job-dir
                #temp_log = os.path.join(self.flow_options.get("job_dir"),"no_devkit_list.log")
                #temp_log = re.sub(r"\\","/",temp_log)
                #temp_log = os.path.abspath(temp_log)
                #temp_hand = file(temp_log,"a")
                #temp_hand.write(same_ldf_dir+"\n")
                #temp_hand.close()
                #-------------------------
                if self.flow_options.get("family"):
                    self.kwargs["DeviceFamily"] = self.flow_options.get("family")
    ...
    def downloadLib(self,lib_name):
        ...
        '''
        try:
            SVN_DIR = "http://linux12v/Platform/trunk/DMSLib"
            ...
            file_lock = file(os.path.join(V_LIB,"lock.file"),"w")
            file_lock.close()
            cmd = "svn export "+lib_name_dir+" "+V_LIB +" --force --username=public --password=lattice --no-auth-cache --non-interactive"
            print cmd
            os.system(cmd)
            os.remove(os.path.join(V_LIB,"lock.file"))
