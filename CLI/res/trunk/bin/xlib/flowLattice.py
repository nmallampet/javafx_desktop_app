    ...
    def merge_local_options(self):
        ...
        if self.run_ice:
            ...
            if not len_info_file: # No info file found
                ldf_file = glob.glob(os.path.join(self.src_design, "par", "*.ldf"))
                if not ldf_file:
                    ldf_file = glob.glob(os.path.join(self.src_design, "*.ldf"))
                ...
                if xTools.get_fname(real_info_file) == "_qas":
                    new_local_options = qas.get_local_bqs_options(self.conf, local_options, self.src_design)
