    ...
    try:
        for foo in os.listdir(cur_dir):
            ...
            if os.path.isfile(abs_foo):
                ...
                elif fext.lower() in (".ldf", ".lpf", ".sty"):
...
def run_ldf_file(tcl_file, ldf_file, process_task, flow_settings=list()):
    """
    ...
    """
    tcl_lines = list()
    tcl_lines.append('prj_project open "%s"' % ldf_file)
    for setting in flow_settings:
        tcl_lines.append(setting)
    tcl_lines.append("prj_project save")
    for (process, task) in process_task:
        if task:
            task = "-task %s" % task
        tcl_lines.append("prj_run %s %s -forceOne" % (process, task))
    tcl_lines.append("prj_project save")
    tcl_lines.append("prj_project close")
...
def get_diamond_version():
    sts, text = xTools.get_status_output("trce")
...
def update_diamond_version(ldf_file, current_version):
    ...
    if float(ldf_version) <= float(current_version):
        return
    ...
    ldf_lines = xTools.get_original_lines(ldf_file)
...
def update_synthesis_tool(ldf_file, synthesis):
    """
...
class UpdateLDF:
    ...
    def update_node_file(self, src_node_file, dst_node_file, property_name):
        ...
        # dst_lines = list()
        # p = re.compile("^workdir\s*=\s*(.+)", re.I)
        # for item in open(src_node_file):
        #     item = item.rstrip()
        #     m = p.search(item)
        #     if not m:
        #         dst_lines.append(item)
        #     else:
        #         new_workdir = os.path.join(os.getcwd(), os.path.basename(m.group(1)))
        #         dst_lines.append("workdir = %s" % xTools.win2unix(new_workdir, use_abs=0))
...
class CreateDiamondProjectFile:
    ...
    def __init__(self, flow_options):
        ...
        for item in ("sim_map_vlg", "sim_map_vhd", "sim_par_vlg", "sim_par_vhd", "sim_all"):
    ...
    def process(self):
        ...
        if self.src_files or self.edf_file or self.ldf_file:
            ...
            if sts:
                return 1
            self.ldf_dict = parse_ldf_file(self.final_ldf_file)
    ...
    def generate_ldf_file(self):
        ...
        if not self.devkit:
            ...
            return 1
        _t_line = 'prj_project new -name "%s" -impl "%s" ' % (self.project_name, self.impl_name)
        if self.synthesis:
        ...
        for item_list in _src_files:
            ...
            tcl_lines.append('prj_src add "%s" %s' % (real_file, lib_name))
        if self.update_pmi_files(_src_files):
            return 1
        tcl_lines.append('prj_project save %s' % self.final_ldf_file)
        tcl_lines.append("prj_project close")
    ...
    def add_strategy_and_synthesis(self):
        ...
        add_lines = list()
        add_lines.append('prj_project open "%s"' % self.final_ldf_file)
        if self.ldf_file:
            if self.devkit: # update it
                add_lines.append('prj_dev set -part %s' % self.devkit.upper())

        if self.synthesis:
            # add_lines.append('prj_syn set "%s"' % self.synthesis)
            # /*
            #  * Not all device can support both synthesis and lse
            #  * so change the synthesis tool name in ldf file directly
            #  * rather than use the tcl command
            #  */
            update_synthesis_tool(self.final_ldf_file, self.synthesis)
        ...
        if self.strategy:
            ...
            add_lines.append('prj_strgy copy -from "%s" -name "%s" -file "%s.sty"' % (gui_name, sty_name, sty_name))
            add_lines.append('prj_strgy set "%s"' % sty_name)
        else:
            ...
            if sty_name in ("Timing", "Area", "Quick"):
                ...
                add_lines.append('prj_strgy copy -from "%s" -name "%s" -file "%s.sty"' % (sty_name, new_sty, new_sty))
                add_lines.append('prj_strgy set "%s"' % new_sty)
                sty_name = new_sty
        pre_set = 'prj_strgy set_value -strategy %s' % sty_name
        ...
        add_lines.append("prj_project save")
        if self.dry_run:
            pass
        elif self.sim_rtl:
            pass
        ...
        add_lines.append("prj_project save")
        add_lines.append("prj_project close")
...
def parse_ldf_file(ldf_file):
    import readXML
    return readXML.parse_ldf_file(ldf_file=ldf_file)
...
class RunTclFlow:
    ...
    def update_flow_options_for_export_cmd(self):
        ...
        if support_flow == "10": # outside in GUI: jedec, inner TCL command: bitstream
            if self.flow_options.get("run_export_jedec"):
                ...
                self.flow_options["export_bitstream"] = 1
        else:
            if self.flow_options.get("run_export_jedec"):
                if support_flow[0] == "0":
    ...
    def run_fmax_seed_flow(self):
        ...
        for i, fmax in enumerate(self.fmax_sweep):
            ...
            for ii, seed in enumerate(self.seed_sweep):
                ...
                pre_set = "prj_strgy set_value -strategy %s" % self.sty_name
                flow_settings = ["%s par_place_iterator_start_pt=%d par_save_best_result=1" % (pre_set, seed)]
                sts = run_ldf_file(run_mark+".tcl", self.final_ldf_file, process_task, flow_settings)
                self.run_ncl_flow()
    ...
    def run_fmax_flow(self):
        ...
        for i, fmax in enumerate(self.fmax_sweep):
            ...
            xTools.say_it("--Launch Test flow: %s" % run_mark)
            process_task = get_task_list(self.flow_options, dict(run_map=1, run_par_trace=1))
            sts = run_ldf_file(run_mark+".tcl", self.final_ldf_file, process_task)
            self.run_ncl_flow()
            move_bak_results(run_mark+".tcl", self.impl_dir, run_mark, sts)
            if self.smoke:
                return
...
def run_scuba_by_file(src_file, run_scuba):
    ...
    # lpc_file = bn + ".lpc"
    # srp_file = bn + ".srp"
    # if os.path.isfile(lpc_file):
    #     sts, lpc_dict = xTools.get_conf_options(lpc_file)
    #     if sts:
    #         return [1, 1]
    #     ori_scuba_cmd_line = lpc_dict.get("Command", dict()).get("cmd_line")
    #     if not ori_scuba_cmd_line:
    #         xTools.say_it("Error. Not found scuba command line in %s" % os.path.abspath(lpc_file))
    #         return [1, 1]
    #     ori_scuba_cmd_line = [ori_scuba_cmd_line]
    # else:
    ...
    #
    #     ori_scuba_cmd_line = xReport.file_simple_parser(scuba_src_site, [re.compile("\S+scuba\.exe(.+)")], 80)
    #     if not ori_scuba_cmd_line: # maybe it is not a model file
    #         return
    # /*
    #  * Since lpc is not the standard ini file format (=mem), Jason agree to skip the lpc file because it is redundant.
    #  */
    srp_file = bn + ".srp"
    if os.path.isfile(srp_file):
        scuba_src_site = srp_file
