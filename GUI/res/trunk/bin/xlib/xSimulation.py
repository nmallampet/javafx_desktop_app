...
class RunSimulationFlow:
    ...
    def process(self):
        ...
        if task_list:
            ...
            sts = xLattice.run_ldf_file("foundry_flow.tcl", self.final_ldf_file, task_list)
            if sts:
    ...
    def flatten_options(self):
        ...
        if self.dev_lib:
            ...
            if self.run_modelsim or self.run_questasim or self.run_riviera:
                ...
                self.dev_lib = os.path.join(foundry_path, "..", "active-hdl", "Vlib", "ovi_" + map_lib_name, "ovi_"+ map_lib_name+".lib")
    ...
    def create_dev_lib(self, map_lib_name):
        ...
        for hdl_type in ("vhdl", "verilog"):
            my_create = simLibrary.GetSimulationLibrary(_diamond, self.sim_vendor_name, self.sim_vendor_bin,
                                                        hdl_type, map_lib_name, self.conf)
            my_create.process()
    ...
    def run_syn_vlg_simulation(self, sim_path):
        if self.synthesis == "lse":
            ...
            if source_files:
                ...
                xTools.say_it("Error. Not found _prim.v file for post lse simulation flow")
                return 1
        else:
            xTools.say_it("TODO: Not support post synplify simulation flow.")
            return
                    ...
                    if start_source:
                        ...
                        if line_list[0] in ("vlog", "vcom"):
                            ...
                            if not os.path.isfile(real_hdl_file):
                                real_hdl_file = xTools.get_relative_path(hdl_file, os.path.join("../../sim"))
                            if not os.path.isfile(real_hdl_file):
                                pass
                            else:
            ...
            for line in open(self.do_template):
                ...
                if start_source:
                    if self.p_src_end.search(line):
                        ...
                        if not v_v_line:
                            xTools.say_it("Error. Not any source files for running %s" % sim_path)
                            xTools.say_it("")
                            _recov.comeback()
                            return 1
