...
class write_excel:
    ...
    def write_title(self,row):
        ...
        for item in self.show_title:
            ...
            if item.find("ALL_CLK") != -1:
                self.totalSheet.col(col).width = 1*int(0x0d00)/10
            col = col + 1
    ...
    def check_new_statistics(self,src_name,cmp_name):
        if src_name.find('synp') != -1 and cmp_name.find('timing')!= -1:
            return 'timing'
        elif src_name.find('timing') != -1 and cmp_name.find('synp')!= -1:
            return 'timing'
        elif src_name.find('synp') != -1 and cmp_name.find('area')!= -1:
            return 'area'
        elif src_name.find('area') != -1 and cmp_name.find('synp')!= -1:
            return 'area'
        else:
            return 'general'
    ...
    def write_body(self,row):
        ...
        for num,cmp in enumerate(self.cmps):
            ...
            for id2,t in enumerate(self.comp_title):
                    ...
                    #if cmp.csv_file.find('synp')
                    if self.check_new_statistics(self.src.csv_file,cmp.csv_file) =='timing' :
                        if  t=='fmax':
                            if t in self.timing_fmax[num].keys():
                                pass
                            else:
                                self.timing_fmax[num][t] = {}
                                self.timing_fmax[num][t]['more0'] = 0
                                self.timing_fmax[num][t]['less5_0'] = 0
                                self.timing_fmax[num][t]['less25_less5'] = 0
                                self.timing_fmax[num][t]['less30_less25'] = 0
                                self.timing_fmax[num][t]['less30'] = 0
                        if t == 'Total_cpu_time_1':
                            if t in self.runtime_30[num].keys():
                                pass
                            else:
                                self.runtime_30[num][t]  = {}
                                self.runtime_30[num][t]['less0']  = 0
                                self.runtime_30[num][t]['more0_50']  = 0
                                self.runtime_30[num][t]['more50']  = 0
                                #self.runtime_31 = {}
                                self.runtime_31[num][t]  = {}
                                self.runtime_31[num][t]['less0']  = 0
                                self.runtime_31[num][t]['more0_25']  = 0
                                self.runtime_31[num][t]['more25']  = 0
                        if t == 'Total_cpu_time_2':
                            if t in self.peak_memory[num].keys():
                                pass
                            else:
                                #self.peak_memory = {}
                                self.peak_memory[t] = {}
                                self.peak_memory[t]['more0'] = 0
                                self.peak_memory[t]['less0'] = 0
                    elif self.check_new_statistics(self.src.csv_file,cmp.csv_file) =='area' :
                        if  t=='LUT':
                            if t in self.area_lut[num].keys():
                                pass
                            else:
                                self.area_lut[num][t] = {}
                                self.area_lut[num][t]['less0'] = 0
                                self.area_lut[num][t]['more0_10'] = 0
                                self.area_lut[num][t]['more10_20'] = 0
                                self.area_lut[num][t]['more20'] = 0
                        if  t=='Slice':
                        
                            if t in self.area_slice[num].keys():
                                pass
                            else:
                                self.area_slice[num][t] = {}
                                self.area_slice[num][t]['less0'] = 0
                                self.area_slice[num][t]['more0_10'] = 0
                                self.area_slice[num][t]['more10_20'] = 0
                                self.area_slice[num][t]['more20'] = 0    
                    ...
                    if t=='Slice' and 'LE' in cmp.old_titles:
                        ...
                        try:
                            ...
                            if t.lower() =='fmax':
                                ...
                                print id3,self.pass_total[id1]['total']
                            ########################Special For FE #########################
                            if self.check_new_statistics(self.src.csv_file,cmp.csv_file) =='timing' :
                                if t=='fmax':
                                    if value >= 0:
                                        self.timing_fmax[id1][t]['more0'] += 1
                                    if -0.05<=value<0:
                                        self.timing_fmax[id1][t]['less5_0'] += 1
                                    if -0.25<=value<-0.05:
                                        self.timing_fmax[id1][t]['less25_less5'] += 1
                                    if -0.30<=value<-0.25:
                                        self.timing_fmax[id1][t]['less30_less25'] += 1
                                    if value<-0.3:
                                        self.timing_fmax[id1][t]['less30'] += 1
                                    
                                if t=='Total_cpu_time_1':
                                    pass
                                if t=='Total_cpu_time_2':
                                    pass
                            if self.check_new_statistics(self.src.csv_file,cmp.csv_file) =='area' :
                                if t=='LUT':
                                    if value <= 0:
                                        self.area_lut[id1][t]['less0'] += 1
                                    if 0<value<=0.1:
                                        self.area_lut[id1][t]['more0_10'] += 1
                                    if 0.1<value<=0.2:
                                        self.area_lut[id1][t]['more10_20'] += 1
                                    if 0.2<=value:
                                        self.area_lut[id1][t]['more20'] += 1
                                if t=='Slice':
                                    if value <= 0:
                                        self.area_slice[id1][t]['less0'] += 1
                                    if 0<value<=0.1:
                                        self.area_slice[id1][t]['more0_10'] += 1
                                    if 0.1<value<=0.2:
                                        self.area_slice[id1][t]['more10_20'] += 1
                                    if 0.2<=value:
                                        self.area_slice[id1][t]['more20'] += 1
    ...
    def write_fe2(self,sheet,row,col,value,threshold='',percent=''):
        style_dict = {'lineStyle':'2222'}
        if percent:
            style_dict.update({'myFormat':'0.00%'})
        body_style_green = style(lineStyle='2222',cellFontColour='green')
        body_style_green = body_style_green.createStyle()
        
        body_style_red = style(lineStyle='2222',cellFontColour='red')
        body_style_red = body_style_red.createStyle()
        ...
        if not threshold:
            ...
            style_dict.update({'cellFontColour':'red'})
        else:
             style_dict.update({'cellFontColour':'green'})
        body_style = style(lineStyle=style_dict.get('lineStyle',''),myFormat=style_dict.get('myFormat',''),cellFontColour=style_dict.get('cellFontColour',''))
        body_style = body_style.createStyle()
        sheet.write(row,col,value,body_style)
    def write_fe(self,sheet,row,col,value,threshold='',percent='',reverse=0):
        ...
        if percent:
            body_style_green = style(lineStyle='2222',cellFontColour='green',myFormat='0.00%')
            body_style_green = body_style_green.createStyle()
            
            body_style_red = style(lineStyle='2222',cellFontColour='red',myFormat='0.00%')
            body_style_red = body_style_red.createStyle()
            
            body_style = style(lineStyle='2222',myFormat='0.00%')
            body_style = body_style.createStyle()
            
        if not reverse:   
            if not threshold:
                body_style2 = body_style
            elif value<threshold:
                body_style2 = body_style_red
            else:
                body_style2 = body_style_green
        else:
            if not threshold:
                body_style2 = body_style
            elif value>threshold:
        ...
        sheet.write(row,col,value,body_style2)
    def write_win_tie(self,row):
        ...
        ########################Write new for FE############################
            ...
            row = row + 10
            if 'fmax' in self.comp_title:
                if self.timing_fmax[id2]:
                    
                    col = len(self.original_title)+len(self.cmps)*len(self.original_title[1:])+ \
                          id2*2*len(self.comp_title)
                    self.totalSheet.write(row, col, '',body_style)
                    self.totalSheet.write(row+1, col, '>0%',body_style)
                    self.totalSheet.write(row+2, col, '-5%~0%',body_style)
                    self.totalSheet.write(row+3, col, '-25%~-5%',body_style)
                    self.totalSheet.write(row+4, col, '-30%~-25%',body_style)
                    self.totalSheet.write(row+5, col, '<-30%',body_style)
                    self.totalSheet.write(row, col+1, 'fmax',body_style)
                    self.totalSheet.write(row, col+2, 'percent',body_style)
                    total_num = self.timing_fmax[id2]['fmax']['more0'] + self.timing_fmax[id2]['fmax']['less5_0'] \
                                +self.timing_fmax[id2]['fmax']['less25_less5'] +self.timing_fmax[id2]['fmax']['less30_less25'] \
                                +self.timing_fmax[id2]['fmax']['less30']
                    total_num = float(total_num)
                    self.write_fe(self.totalSheet,row+1,col+1,self.timing_fmax[id2]['fmax']['more0'])
                    self.write_fe(self.totalSheet,row+2,col+1,self.timing_fmax[id2]['fmax']['less5_0'])
                    self.write_fe(self.totalSheet,row+3,col+1,self.timing_fmax[id2]['fmax']['less25_less5'])
                    self.write_fe(self.totalSheet,row+4,col+1,self.timing_fmax[id2]['fmax']['less30_less25'])
                    self.write_fe(self.totalSheet,row+5,col+1,self.timing_fmax[id2]['fmax']['less30'])
                    
                    self.write_fe(self.totalSheet,row+1,col+2,self.timing_fmax[id2]['fmax']['more0']/total_num,0.5,1)
                    self.write_fe(self.totalSheet,row+2,col+2,self.timing_fmax[id2]['fmax']['less5_0']/total_num,0.25,1)
                    self.write_fe(self.totalSheet,row+3,col+2,self.timing_fmax[id2]['fmax']['less25_less5']/total_num,0.15,1)
                    self.write_fe(self.totalSheet,row+4,col+2,self.timing_fmax[id2]['fmax']['less30_less25']/total_num,0.05,1)
                    self.write_fe(self.totalSheet,row+5,col+2,self.timing_fmax[id2]['fmax']['less30']/total_num,0.05,1)
                    row = row + 7
                #except:
                #    pass
            if 'LUT' in self.comp_title:
                if self.area_lut[id2]:
                    col = len(self.original_title)+len(self.cmps)*len(self.original_title[1:])+ \
                          id2*2*len(self.comp_title)
                    self.totalSheet.write(row, col, '',body_style)
                    self.totalSheet.write(row+1, col, '<0%',body_style)
                    self.totalSheet.write(row+2, col, '0%~10%',body_style)
                    self.totalSheet.write(row+3, col, '10%~20%',body_style)
                    self.totalSheet.write(row+4, col, '>20%',body_style)
                    self.totalSheet.write(row, col+1, 'LUT',body_style)
                    self.totalSheet.write(row, col+2, 'percent',body_style)
                    total_num = self.area_lut[id2]['LUT']['less0'] + self.area_lut[id2]['LUT']['more0_10']  \
                                +self.area_lut[id2]['LUT']['more10_20'] + self.area_lut[id2]['LUT']['more20']
                               
                    total_num = float(total_num)
                    
                    self.write_fe(self.totalSheet,row+1,col+1,self.area_lut[id2]['LUT']['less0'])
                    self.write_fe(self.totalSheet,row+2,col+1,self.area_lut[id2]['LUT']['more0_10'])
                    self.write_fe(self.totalSheet,row+3,col+1,self.area_lut[id2]['LUT']['more10_20'])
                    self.write_fe(self.totalSheet,row+4,col+1,self.area_lut[id2]['LUT']['more20'])
                    
                    
                    self.write_fe(self.totalSheet,row+1,col+2,self.area_lut[id2]['LUT']['less0']/total_num,0.5,1)
                    self.write_fe(self.totalSheet,row+2,col+2,self.area_lut[id2]['LUT']['more0_10']/total_num,0.35,1)
                    self.write_fe(self.totalSheet,row+3,col+2,self.area_lut[id2]['LUT']['more10_20']/total_num,0.1,1)
                    self.write_fe(self.totalSheet,row+4,col+2,self.area_lut[id2]['LUT']['more20']/total_num,0.05,1)
                    row = row + 6
            if 'Slice' in self.comp_title: 
                if self.area_slice[id2]:
                    col = len(self.original_title)+len(self.cmps)*len(self.original_title[1:])+ \
                          id2*2*len(self.comp_title)
                    self.totalSheet.write(row, col, '',body_style)
                    self.totalSheet.write(row+1, col, '<0%',body_style)
                    ...
                    self.totalSheet.write(row+3, col, '10%~20%',body_style)
                    self.totalSheet.write(row+4, col, '>20%',body_style)
                    self.totalSheet.write(row, col+1, 'Slice',body_style)
                    self.totalSheet.write(row, col+2, 'percent',body_style)
                    total_num = self.area_slice[id2]['Slice']['less0'] + self.area_slice[id2]['Slice']['more0_10'] \
                                +self.area_slice[id2]['Slice']['more10_20'] +self.area_slice[id2]['Slice']['more20']
                                
                    total_num = float(total_num)
                    self.write_fe(self.totalSheet,row+1,col+1,self.area_slice[id2]['Slice']['less0'])
                    self.write_fe(self.totalSheet,row+2,col+1,self.area_slice[id2]['Slice']['more0_10'])
                    self.write_fe(self.totalSheet,row+3,col+1,self.area_slice[id2]['Slice']['more10_20'])
                    self.write_fe(self.totalSheet,row+4,col+1,self.area_slice[id2]['Slice']['more20'])
                    
                    
                    self.write_fe(self.totalSheet,row+1,col+2,self.area_slice[id2]['Slice']['less0']/total_num,0.5,1)
                    self.write_fe(self.totalSheet,row+2,col+2,self.area_slice[id2]['Slice']['more0_10']/total_num,0.35,1)
                    self.write_fe(self.totalSheet,row+3,col+2,self.area_slice[id2]['Slice']['more10_20']/total_num,0.1,1)
                    self.write_fe(self.totalSheet,row+4,col+2,self.area_slice[id2]['Slice']['more20']/total_num,0.05,1)
                    row = row + 6
    ...
    def return_dict(self):
        for id1,cmp in enumerate(self.cmps):
            ...
            others = [
                self.timing_fmax[id1],
                self.runtime_30[id1],
                self.runtime_31[id1],
                self.peak_memory[id1],
                self.area_lut[id1],
                self.area_slice[id1],
                self.pass_total[id1]
                ]
    ...
    def write_note_Note(self,row):
        ...
        #self.totalSheet.write(row-1, col, value1)
def write_fe(sheet,row,col,value,threshold='',percent='',reverse=0):
...
def write_total_summay(statistic='',line='',summary={}): #only for QoR
        ...
        # add pass/fail: {sheetname:[{tilte(slice):{  key(less15_5):value(1)},  },{title(slice):value(0.0%)},{'Pass/Total':**}]}
        #--------------------------------------------------------------#
        ...
        if not line:
            line = 25
        col = 1+3
        #priority=  ['sapphire','xo3l','xo2','ecp3','xo','xp2','ecp2','Others']
        priority = []
        ...
        for sheet_f in summary.keys():
            ...
            if f in priority:
                ...
                priority_data.setdefault(sheet_f,[]).append(sheet_f)      
        #for p_id,item in enumerate(priority):
        for p_id,item in enumerate(priority_data.keys()):
            col = 1+3
            id4 = 0
            ...
            statistic.write(line-1,col-3,''+str(p_id),bold_black)
            ...
            for row_title in key_order:
                ...
                for id2,s_t in enumerate(statistic_tie_list[3:]):
                    ...
                    id4 = id2
                id1 = id1 + 1
            #line = line+3+id2+id3+3
            next_col = next_col + 2
            line = line + 8
            #return line+1+id4 
            
            #-----------------Special Support for FE-------------------#
            ...
            for s_d in others:
                if s_d:
                    if 'pass' in s_d.keys():
                        continue
                    for id_k,s_d_k in enumerate(s_d.keys()):
                        statistic.write(line+id_k,next_col,'',bold_title)
                        statistic.write(line+id_k,next_col+1,s_d_k,bold_title)
                        statistic.write(line+id_k,next_col+2,'Percent',bold_title)
                        if s_d_k == 'fmax':
                            '''self.totalSheet.write(row, col, '',body_style)
                            self.totalSheet.write(row+1, col, '>0%',body_style)
                            self.totalSheet.write(row+2, col, '-5%~0%',body_style)
                            ...
                            total_num = self.timing_fmax[id2]['fmax']['more0'] + self.timing_fmax[id2]['fmax']['less5_0'] \
                                        ...
                                        +self.timing_fmax[id2]['fmax']['less30']
                            total_num = float(total_num)'''
                            total_num = s_d['fmax']['more0'] + s_d['fmax']['less5_0'] + \
                                        s_d['fmax']['less25_less5']+ s_d['fmax']['less30_less25'] + s_d['fmax']['less30']
                            total_num = float(total_num)
                            write_fe(statistic,line+id_k+1,next_col,'>0%')
                            write_fe(statistic,line+id_k+2,next_col,'-5%~0%')
                            write_fe(statistic,line+id_k+3,next_col,'-25%~-5%')
                            write_fe(statistic,line+id_k+4,next_col,'-30%~-25%')
                            write_fe(statistic,line+id_k+5,next_col,'<-30%')
                            
                            write_fe(statistic,line+id_k+1,next_col+1,s_d['fmax']['more0'])
                            write_fe(statistic,line+id_k+2,next_col+1,s_d['fmax']['less5_0'])
                            write_fe(statistic,line+id_k+3,next_col+1,s_d['fmax']['less25_less5'])
                            write_fe(statistic,line+id_k+4,next_col+1,s_d['fmax']['less30_less25'])
                            write_fe(statistic,line+id_k+5,next_col+1,s_d['fmax']['less30'])
                            
                            write_fe(statistic,line+id_k+1,next_col+2,s_d['fmax']['more0']/total_num,0.5,1)
                            write_fe(statistic,line+id_k+2,next_col+2,s_d['fmax']['less5_0']/total_num,0.25,1)
                            write_fe(statistic,line+id_k+3,next_col+2,s_d['fmax']['less25_less5']/total_num,0.15,1)
                            write_fe(statistic,line+id_k+4,next_col+2,s_d['fmax']['less30_less25']/total_num,0.05,1)
                            write_fe(statistic,line+id_k+5,next_col+2,s_d['fmax']['less30']/total_num,0.05,1)
                            
                            
                            next_col += 4
                            statistic.write(line+id_k,next_col,'ACC',bold_title)
                            statistic.write(line+id_k,next_col+1,'Standard',bold_title)
                            statistic.write(line+id_k,next_col+2,'fmax',bold_title)
                            write_fe(statistic,line+id_k+1,next_col,'>0%')
                            write_fe(statistic,line+id_k+2,next_col,'>-5%')
                            write_fe(statistic,line+id_k+3,next_col,'>-25%')
                            write_fe(statistic,line+id_k+4,next_col,'>-30%')
                            write_fe(statistic,line+id_k+5,next_col,'<-30%')
                            
                            write_fe(statistic,line+id_k+1,next_col+1,'50.00%')
                            write_fe(statistic,line+id_k+2,next_col+1,'75.00%')
                            write_fe(statistic,line+id_k+3,next_col+1,'90.00%')
                            write_fe(statistic,line+id_k+4,next_col+1,'95.00%')
                            write_fe(statistic,line+id_k+4,next_col+1,'100.00%')
                            
                            
                            write_fe(statistic,line+id_k+1,next_col+2,s_d['fmax']['more0']/total_num,'',1)
                            write_fe(statistic,line+id_k+2,next_col+2,(s_d['fmax']['more0']+s_d['fmax']['less5_0'])/total_num,'',1)
                            write_fe(statistic,line+id_k+3,next_col+2,(s_d['fmax']['more0']+s_d['fmax']['less5_0']+s_d['fmax']['less25_less5'])/total_num,'',1)
                            write_fe(statistic,line+id_k+3,next_col+2,(s_d['fmax']['more0']+s_d['fmax']['less5_0']+s_d['fmax']['less25_less5']+s_d['fmax']['less30_less25'])/total_num,'',1)
                            
                            write_fe(statistic,line+id_k+4,next_col+2,s_d['fmax']['less30']/total_num,'',1)
                            
                            
                        if s_d_k == 'LUT':
                            total_num = s_d['LUT']['less0'] + s_d['LUT']['more0_10'] + s_d['LUT']['more10_20']+ s_d['LUT']['more20']
                            total_num = float(total_num)
                            
                            
                            write_fe(statistic,line+id_k+1,next_col,'<0%')
                            write_fe(statistic,line+id_k+2,next_col,'0~10%')
                            write_fe(statistic,line+id_k+3,next_col,'10%~20%')
                            write_fe(statistic,line+id_k+4,next_col,'>20%')
                            
                            write_fe(statistic,line+id_k+1,next_col+1,s_d['LUT']['less0'])
                            write_fe(statistic,line+id_k+2,next_col+1,s_d['LUT']['more0_10'])
                            write_fe(statistic,line+id_k+3,next_col+1,s_d['LUT']['more10_20'])
                            write_fe(statistic,line+id_k+4,next_col+1,s_d['LUT']['more20'])
                            
                            
                            write_fe(statistic,line+id_k+1,next_col+2,s_d['LUT']['less0']/total_num,0.5,1)
                            write_fe(statistic,line+id_k+2,next_col+2,s_d['LUT']['more0_10']/total_num,0.35,1)
                            write_fe(statistic,line+id_k+3,next_col+2,s_d['LUT']['more10_20']/total_num,0.1,1)
                            write_fe(statistic,line+id_k+4,next_col+2,s_d['LUT']['more20']/total_num,0.05,1)
                            
                            next_col += 4
                            statistic.write(line+id_k,next_col,'ACC',bold_title)
                            statistic.write(line+id_k,next_col+1,'Standard',bold_title)
                            statistic.write(line+id_k,next_col+2,'LUT',bold_title)
                            write_fe(statistic,line+id_k+1,next_col,'<0%')
                            write_fe(statistic,line+id_k+2,next_col,'<10%')
                            write_fe(statistic,line+id_k+3,next_col,'<20%')
                            write_fe(statistic,line+id_k+4,next_col,'>20%')
                            
                            write_fe(statistic,line+id_k+1,next_col+1,'50.00%')
                            write_fe(statistic,line+id_k+2,next_col+1,'85.00%')
                            write_fe(statistic,line+id_k+3,next_col+1,'95.00%')
                            write_fe(statistic,line+id_k+4,next_col+1,'100.00%')
                            
                            
                            write_fe(statistic,line+id_k+1,next_col+2,s_d['LUT']['less0']/total_num,'',1)
                            write_fe(statistic,line+id_k+2,next_col+2,(s_d['LUT']['less0']+s_d['LUT']['more0_10'])/total_num,'',1)
                            write_fe(statistic,line+id_k+3,next_col+2,(s_d['LUT']['less0']+s_d['LUT']['more0_10']+s_d['LUT']['more10_20'])/total_num,'',1)
                            write_fe(statistic,line+id_k+4,next_col+2,s_d['LUT']['more20']/total_num,'',1)
                            
                            
                            
                        if s_d_k == 'Slice':
                            total_num = s_d['Slice']['less0'] + s_d['Slice']['more0_10'] + s_d['Slice']['more10_20']+ s_d['Slice']['more20']
                            total_num = float(total_num)
                            write_fe(statistic,line+id_k+1,next_col,'<0%')
                            write_fe(statistic,line+id_k+2,next_col,'0~10%')
                            write_fe(statistic,line+id_k+3,next_col,'10%~20%')
                            ...
                            write_fe(statistic,line+id_k+1,next_col+1,s_d['Slice']['less0'])
                            write_fe(statistic,line+id_k+2,next_col+1,s_d['Slice']['more0_10'])
                            write_fe(statistic,line+id_k+3,next_col+1,s_d['Slice']['more10_20'])
                            write_fe(statistic,line+id_k+4,next_col+1,s_d['Slice']['more20'])
                            
                            
                            write_fe(statistic,line+id_k+1,next_col+2,s_d['Slice']['less0']/total_num,0.5,1)
                            write_fe(statistic,line+id_k+2,next_col+2,s_d['Slice']['more0_10']/total_num,0.35,1)
                            write_fe(statistic,line+id_k+3,next_col+2,s_d['Slice']['more10_20']/total_num,0.1,1)
                            write_fe(statistic,line+id_k+4,next_col+2,s_d['Slice']['more20']/total_num,0.05,1)
                            
                            next_col += 4
                            statistic.write(line+id_k,next_col,'ACC',bold_title)
                            statistic.write(line+id_k,next_col+1,'Standard',bold_title)
                            statistic.write(line+id_k,next_col+2,'Slice',bold_title)
                            write_fe(statistic,line+id_k+1,next_col,'<0%')
                            write_fe(statistic,line+id_k+2,next_col,'<10%')
                            write_fe(statistic,line+id_k+3,next_col,'<20%')
                            write_fe(statistic,line+id_k+4,next_col,'>20%')
                            ...
                            write_fe(statistic,line+id_k+1,next_col+2,s_d['Slice']['less0']/total_num,'',1)
                            write_fe(statistic,line+id_k+2,next_col+2,(s_d['Slice']['less0']+s_d['Slice']['more0_10'])/total_num,'',1)
                            write_fe(statistic,line+id_k+3,next_col+2,(s_d['Slice']['less0']+s_d['Slice']['more0_10']+s_d['Slice']['more10_20'])/total_num,'',1)
                            write_fe(statistic,line+id_k+4,next_col+2,s_d['Slice']['more20']/total_num,'',1)
                        ...
                        if kk and 0 :
                            temp_total_num = 0.0
                            for id_k_k,s_d_k_k in enumerate(kk.keys()):
                                temp_total_num = temp_total_num + kk.get(s_d_k_k,0)
                            temp_total_num = float(temp_total_num)
                            for id_k_k,s_d_k_k in enumerate(kk.keys()):
                                statistic.write(line+id_k+1+id_k_k,next_col,s_d_k_k,bold_title)
                                statistic.write(line+id_k+1+id_k_k,next_col+1,kk.get(s_d_k_k),bold_title)
                                statistic.write(line+id_k+1+id_k_k,next_col+2,kk.get(s_d_k_k,0)/temp_total_num,statistic_percent)
                                write_fe(statistic,line+id_k+1+id_k_k,next_col+3,'AAA')
                    next_col = next_col + 4
