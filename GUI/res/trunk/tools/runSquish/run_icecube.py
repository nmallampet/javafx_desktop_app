...
CUBE_EXTERNAL_ENV_KEY = "EXTERNAL_ICECUBE_PATH"
...
class RunSquishCase:
    ...
    def run_option_parser(self):
        ...
        parser.add_option("--icecube", help="specify iCEcube path")
    ...
    def _check_options(self):
        ...
        self.icecube = get_real_value(self.icecube, CUBE_EXTERNAL_ENV_KEY)
        self.squish = get_real_value(self.squish, SQUISH_EXTERNAL_ENV_KEY)

        if xTools.not_exists(self.icecube, "iCECube Path"):
            return 1
    ...
    def run_batch_file(self):
        ...
        batch_kwargs = {"squishserver" : self.squish_server, "squishrunner" : self.squish_runner,
                        "path_to_aut" : os.path.join(self.icecube, "sbt_backend", "bin", "win32", "opt"),
        ...
        if self.on_win:
            batch_kwargs["set_path"] = r'set PATH=%%PATH%%;%s;%s\bin' % (os.path.join(self.icecube, "sbt_backend", "bin", "win32", "opt"), self.squish)
            ...
            batch_kwargs["name_of_aut"] = "ProjectNavigator2.exe"
        else:
            batch_kwargs["run_server"] = "%s &" % self.squish_server
            batch_kwargs["sleep"] = "sleep 5"
            batch_kwargs["name_of_aut"] = "ProjectNavigator2"
