...
# version 4.0 : this program will run till-map first, and than run 50-300 point parallel
################################################################################
...
def option():
    ...
    public_group.add_option("--lse-last", action="store_true",help="If the user set this, the case name will be processed as lse-daily way")
        ...
        if cases == -1:
            ...
            while 1:
                ...
                if run_cmd or case_dict:
                    ...
                    lock.release()
                    print '###',now_value,process_num
                    print int(now_value) < int(process_num)
                    if int(now_value) < int(process_num):
                        #used_number = retrun_process_num()
                        #used_number = -1 # at here, ignore the retrun_process_num
                        ...
                        if 1:
                            ...
                            time.sleep(10)
                            continue
                        #*******************************Check sub design pool***************
                        
                        #1. Iterate all cases, if a design has no till map, select the case
                        #2. Remove the selected design
                        ...
                        for d,cmd_list in temp_case_dict.items():
                            ...
                            if not cmd_list:
                                case_dict.pop(d)
                                continue
                            if cmd_list[0].find('--till-map') != -1:
                                case_dict[d] = cmd_list
                                continue
                            else:
                                sub_cmd = cmd_list[0]
                                case_dict[d] = cmd_list[1:]
                                break
                        lock.release()  
                        if sub_cmd:
                            cmd = sub_cmd
                        elif run_cmd: # used for process multi-process-lulti-machine
                            cmd = run_cmd[0]
                            ...
                            if case_run_over == 1:
                                continue
                            #**********************At here we get a new design from the top design pool**********
                            
                            # first:we need to separate it into 27(till-map + 26 point)
                            # second:run till map
                            # third: when till map is run over, the sub-process should remove it 
                            
                            #*******************************END**************************************************
                            #the first step
                            lock.acquire()
                            separate_design = get_cmd_value(cmd,'design').replace(top_dir,'').strip('\\').strip('/')
                            #--fmax-sweep=260 260 10
                            separate_cmds = [cmd + ' --till-map']+[cmd+' --map-done --fmax-sweep='+str(fmax_r)+' '+str(fmax_r)+' 10' for fmax_r in range(50,310,10)]
                            case_dict.update({separate_design:separate_cmds})
                            lock.release()
                            #the second step
                            run_cmd.remove(cmd)
                            cmd = separate_cmds[0]
                        ...
                        p1 = Process(target=run_case,args=(cmd,lock,max_process,log_file,run_over_file,TIME_flag,case_dict))
                        p1.daemon = True
                        p1.start()
