...
class TA:
    def __init__(self,top_dir='',design='',report_path='',report='check.csv',qas=False,tag='_scratch',rerun_path=os.getcwd(),
        ...
        self.check_dict = dict(
                               ...
                               #jason add following method
                               check_diamond_flow = self.check_diamond_flow
                               )
        ...
        if 1:
            if 1:
                ...
                for design in self.designs:
                    ...
                    #############################################################
                    if not self.logic_result_check:
                        pass
                    else:
                        ...
                        try:
                            ...
                            self.result = False
                            print "Can not execute logic result line"
                            self.return_log.append("Can not execute logic result line")
                    print self.result
                    ###############################################################
    ...
    def check_it2(self, design_path):
        ...
        if not conf_files and not self.conf_file :
            ...
            if trunk_cmd.find('--run-par-trce')!= -1 or self.partrce_check:
                ...
                conf_file_h.write('type = type_auto\n')
           ...
           if conf_files and (not self.conf_file):
                ...
                for conf_f in conf_files:
                    for conf_file_l in conf_file_list:
                        if '.conf' in conf_file_l:
                            conf_file_option = conf_file_l
                        else:
                            conf_file_option = conf_file_l +'.conf'
                        if os.path.basename(conf_f) == conf_file_option:
        ...
        for conf_file in conf_files:
            ...
            for item, value in method_dict.items():
                if value == "1":
                    ...
                    elif re.search('check_diamond_flow',item):
                        self.old_item = item
                        item="check_diamond_flow"                        
                    else:
                        self.old_item = item
                    ...
                    try:
                        ...
                        print e
                        print("Error. Can not execute  method name: %s" % item)
                        self.return_log.append("Error. Can not execute method: %s" % item)
                        self.Comments = 'Can not execute method : %s, Please double check your conf file"' % item
                        self.result = False
    ...
    def check_lines(self):
        ...
        try:
            ...
            print 'In %s:'%self.old_item,'Fail'
            #self.exception_find_flag = 1 #Jason.W added 2016/05/06
            self.logic_result_check_dict[self.old_item] = False
            return
        result_single = "Fail"
    ...
    def check_map(self):
        ...
        try:
            ...
            self.Comments = self.Comments+" ;" + "In %s, the file %s not exist"%(self.old_item,file_name_log)
            print 'In %s:'%self.old_item,'Fail'
            #self.exception_find_flag = 1  #Jason.W changed 2016/05/06
            self.logic_result_check_dict[self.old_item] = False
            return
        #print file_name
        result_single = "Fail"
    ...
    #jason add following method          
    def check_diamond_flow(self):
        '''
            search file:<implemetation dir>/*.*
        '''
        flow_check_dict = {
                             'synp'     :   {'sub_path':'_scratch/*/*.srr', 'check_string':'Mapper successful!'},
                             'lse'      :   {'sub_path':'_scratch/*/synthesis.log', 'check_string':'Elapsed CPU time for LSE flow'},
                             ...
                             'bitstream':   {'sub_path':'_scratch/*/*.bit', 'check_string':'FILE_EXISTS'},
                             'jedec'    :   {'sub_path':'_scratch/*/*.jed', 'check_string':'FILE_EXISTS'},
                             'prom'     :   {'sub_path':'_scratch/*/*.mcs', 'check_string':'FILE_EXISTS'}
        }
        print "Come Into the %s"%self.old_item
        support_lst = flow_check_dict.keys()
        support_lst.sort()
        print ">>>Current support check flow : "  +  str(support_lst)
        check_flow = ""
        method_dict = self.conf_options.get("check_diamond_flow")
        keyss = method_dict.keys()
        keyss.sort()
        temp_cr_note = ""
        for item  in keyss:
        ...
        if not check_flow:
            self.result = False
            self.Comments = self.Comments+" ;" + "check file format wrong."
            print 'In %s:'%self.old_item,'Fail'
            self.exception_find_flag = 1
            self.logic_result_check_dict[self.old_item] = False
            return
        check_list_str = check_flow.split('#')
        check_list_raw = check_list_str[0].split(',')
        check_list = []
        for item in check_list_raw:
            item = item.strip()
            if not item:
                continue
            #should be bypass when trunk ready
            if item == 'prom':
                continue
            check_list.append(item.lower())
        check_list.sort()
        print '>>>Current check list         : ' + str(check_list)
        miss_match = False
        for item in check_list:
            if item not in support_lst:
                miss_match = True
                break
        if miss_match:
            self.result = False
            self.Comments = self.Comments+" ;" + "check_flow value wrong"
            print 'In %s:'%self.old_item,'Fail'
            self.exception_find_flag = 1
            self.logic_result_check_dict[self.old_item] = False
            return         
        ...
        fail_flow = []
        for item in check_list:
            temp_result = self.diamond_file_check(item, flow_check_dict)
            if not temp_result:
                fail_flow.append(item)
        if len(fail_flow) > 0:
            final_result = False
        else:
            final_result = True
        #final result
        print 'In %s:'%self.old_item,final_result
        if final_result and self.result:
           pass
        elif not final_result:
            if not temp_cr_note:
                self.result = False
                self.Comments = self.Comments+" ;" + 'check_diamond_flow'
                self.logic_result_check_dict[self.old_item] = False
        else:
            pass  

    #self.design_path
    def diamond_file_check(self, flow_name, flow_check_dict):
        """
        """
        check_flow = flow_name
        sub_path   = flow_check_dict[flow_name]['sub_path']
        check_str  = flow_check_dict[flow_name]['check_string']
        check_patt = re.compile(check_str, re.I)
        total_path = os.path.join(self.design_path, sub_path)
        file_list = glob.glob(total_path)
        if check_str == "FILE_EXISTS":
            if len(file_list) > 0:
                return True
            else:
                return False
        ...
        file_frh  = file(file_path,"r")
        line_list = file_frh.readlines()
        file_frh.close()
        get_check_string = False
        for line in line_list:
            line = line.strip()
            if not line:
                continue
            check_search = check_patt.search(line)
            if check_search:
                get_check_string = True
                break
        if get_check_string:
            return True
        else:
